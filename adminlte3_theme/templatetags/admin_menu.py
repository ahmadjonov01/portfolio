    ...
    def render(self, context, menus=None):
        ...
        for group in menus:
            ...
            if len(group[key]['children']) > 0:
                ...
                if (request.path == reverse('admin:index')):
                    r += '<li ><a href="%s" class="nav-link">%s <p>%s</p></a></li>\n' % (
        ...
        for app in context['available_apps']:
            ...
            for model in app['models']:
                ...
                if 'admin_url' in model:
                    ...
                    if model['object_name'].title() == 'About':
                        icon = '<i class="fab fa-amilia"></i>'
                        ...
                        if (request.path == url):
                            r += '<a href="%s" class="nav-link active">%s %s</a>' % (url, icon, model['name'])
                        else:
                            r += '<a href="%s" class="nav-link">%s %s</a>' % (url, icon, model['name'])
                    if model['object_name'].title() == 'Num':
                    ...
                    # if model['object_name'].title() == 'Image':
                    #     icon = '<i class="fab fa-amilia"></i>'
                    #     if model['object_name'].title() in self.models_icon:
                    #         if self.models_icon[model['object_name'].title()] != '':
                    #             if re.match(r'\<([a-z]*)\b[^\>]*\>(.*?)\<\/\1\>',
                    ...
                    #     else:
                    #         r += '<a href="%s" class="nav-link">%s %s</a>' % (url, icon, model['name'])
                # if model['object_name'].title() == 'News':
                #     icon = '<i class="nav-icon bi bi-newspaper"></i>'
                #     if model['object_name'].title() in self.models_icon:
                #         if self.models_icon[model['object_name'].title()] != '':
                #             if re.match(r'\<([a-z]*)\b[^\>]*\>(.*?)\<\/\1\>',
                #                         self.models_icon[model['object_name'].title()]):
